...
A: `Slice99` just holds `void *` to access data. If you want to stay type-safe, you can still convert a length with a properly typed pointer to `Slice99` by `Slice99_from_typed_ptr(ptr, len)` and use the multitude of functions it provides (`Slice99_advance`, `Slice99_last`, `Slice99_find`, etc.).
